#!/usr/bin/env python3
import numpy as np
from os import urandom
from pyfinite import ffield
import galois
import pprint as pp
from Crypto.Cipher import AES
import string

#Get change matrixes
#Send 0 Block to the encryption method
#Find what creates the enc_flag block from the enc_zero_block, then create the plaintext using the vectors



FLAG = b'crypto{ABCDEFGHIJKLMNOPQRSTUV}'

GF = galois.GF(2)


X = []

class AES2:
    sbox = (
        
    )
    
    rcon = (0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36)

    # multiple with F(2) under irreduciable polynom
    gmul2 = (
        
    )
    
    # multiple with F(2) under irreduciable polynom
    gmul3 = (
        
    )

    def __init__(self, key):
        self._block_size = 16
        global X
        self._round_keys = self._expand_key([i for i in key])
        X = self._round_keys
        
        self._state = []

    def _transpose(self, m):
        return [m[4 * j + i] for i in range(4) for j in range(4)]

    def _xor(self, a, b):
        return [x ^ y for x, y in zip(a, b)]

    def _expand_key(self, key):
        global KEYS_GLOBAL
        
        round_keys = [key]

        for i in range(10):
            round_key = []
            first = round_keys[i][:4]
            last = round_keys[i][-4:]
            last = last[1:] + [last[0]]
            last = [self.sbox[i] for i in last]

            round_key.extend(self._xor(self._xor(first, last), [self.rcon[i+1], 0, 0, 0]))
            for j in range(0, 12, 4):
                round_key.extend(self._xor(round_key[j:j + 4], round_keys[i][j + 4:j + 8]))
            round_keys.append(round_key)

        for i in range(len(round_keys)):
            round_keys[i] = self._transpose(round_keys[i])

        KEYS_GLOBAL = round_keys
        return round_keys

    def _add_round_key(self, i):
        self._state = self._xor(self._round_keys[i], self._state)

    def _mix_columns(self):
        s = [0] * self._block_size
        for i in range(4):
            s[i] = self.gmul2[self._state[i]] ^ self.gmul3[self._state[i + 4]] ^ self._state[i + 8] ^ self._state[i + 12]
            s[i + 4] = self._state[i] ^ self.gmul2[self._state[i + 4]] ^ self.gmul3[self._state[i + 8]] ^ self._state[i + 12]
            s[i + 8] = self._state[i] ^ self._state[i + 4] ^ self.gmul2[self._state[i + 8]] ^ self.gmul3[self._state[i + 12]]
            s[i + 12] = self.gmul3[self._state[i]] ^ self._state[i + 4] ^ self._state[i + 8] ^ self.gmul2[self._state[i + 12]]
        self._state = s

    def _shift_rows(self):
        self._state = [
            self._state[0], self._state[1], self._state[2], self._state[3],
            self._state[5], self._state[6], self._state[7], self._state[4],
            self._state[10], self._state[11], self._state[8], self._state[9],
            self._state[15], self._state[12], self._state[13], self._state[14]
        ]

    def _sub_bytes(self):
        self._state = [self.sbox[i] for i in self._state]

    def _encrypt_block(self):
        self._add_round_key(0)
        
        for i in range(1, 10):
            self._sub_bytes()
            self._shift_rows()
            self._mix_columns()
            self._add_round_key(i)

        self._sub_bytes()
        self._shift_rows()
        self._add_round_key(10)

    def encrypt(self, plaintext):
        ciphertext = b''

        self._state = self._transpose([c for c in plaintext])
        self._encrypt_block()
        ciphertext += bytes(bytearray(self._transpose(self._state)))

        return ciphertext




def e(aes, message):

 
    if len(message) != 16:
        return {'error': 'Message length must be exactly 16 bytes'}

    return aes.encrypt(message)

def e_f(aes):

    
    padding_length = 16 - (len(FLAG) % 16)
    padded_flag = FLAG + bytes([padding_length]) * padding_length

    encrypted_flag  = b''
    for i in range(0, len(padded_flag), 16):
        encrypted_flag += aes.encrypt(padded_flag[i:i + 16])

    return encrypted_flag

t = AES2.sbox

key1 = urandom(16)
aes1 = AES2(key1)
key2 = urandom(16)
aes2 = AES2(key2)

##tt = []
##i = 0
##while True:
##    c = t[i]
##    if c in tt:
##        break
##
##    tt.append(c)
##    i = c

# sbox[20] => 20
##enc_flag = e_f()

def hex_diff(x, y):
    
    x = bin(int(x, 16))[2:]
    y = bin(int(y, 16))[2:]
    
    x = x.zfill(128)
    y = y.zfill(128)

    same = 0
    for i in range(128):
        if x[i] == y[i]:
            same += 1

    return same

def num_to_bits(x):
    x = bin(x)[2:]
    x = x.zfill(128)
    return x

def hex_to_bits(x):
    x = int.from_bytes(x, "little")

    x = bin(x)[2:]

    x = x.zfill(128)
    return x


def bits_to_hex(x):
    x = int(x, 2)

    x = x.to_bytes(16, "little")
    

    return x


def xor(x, y):
    x = int.from_bytes(x, "little")
    y = int.from_bytes(y, "little")

    z = x ^ y

    return z.to_bytes(16, "little")

FLAG = b'crypto{ABCDEFGHIJKLMNOPQRSTUV}'


# Create identity matrix for it
ee = []
eee = []
zero = 0
zero_enc = e(aes1, zero.to_bytes(16, "little"))

for i in range(128):
    n = 1 << i
    ee.append(e(aes1, n.to_bytes(16, "little")))
    eee.append(n.to_bytes(16, "little"))
    
changes = []
for i in range(128):
    x = int.from_bytes(ee[i], "little") ^ int.from_bytes(zero_enc, "little")
    changed = x.to_bytes(16, "little")
    #changed = hex_to_bytes(changed)
    changes.append(changed)


enc_flag = e_f(aes2)[:16]
enc_zero = e(aes2, b"\x00"*16)

enc_xored = xor(enc_flag, enc_zero)

# from changes to enc_xored
mat_eqs = []
for i in range(128):
    curr_row = []
    c = hex_to_bits(changes[i])
    for j in range(128):
        curr_row.append(int(c[j]))
    
    mat_eqs.append(curr_row)

truth_vector = [int(i) for i in list(hex_to_bits(enc_xored))]

mat_eqs = GF(mat_eqs).T
truth_vector = GF(truth_vector)

pols = np.linalg.solve(mat_eqs, truth_vector)

mat_e = []
for i in range(128):
    curr_row = []
    n = 1 << i
    c = num_to_bits(n)
    for j in range(128):
        curr_row.append(int(c[j]))
    mat_e.append(curr_row)


mat_e = GF(mat_e)

f = np.dot(mat_e, pols)

s = ""
for i in f:
    s += str(i.item())


print(bits_to_hex(s))

    
# test
##test = (1 << 127) ^ (1 << 126) ^ (1 << 125) ^ (1 << 100) ^ (1 << 5)
##t = e(aes1, test.to_bytes(16, "little")).hex()
##
##cc = zero_enc
##cc = xor(cc, changes[127])
##cc = xor(cc, changes[126])
##cc = xor(cc, changes[125])
##cc = xor(cc, changes[100])
##cc = xor(cc, changes[5])
##
##print(t)
##print(cc.hex())






#print("  " + flag)




##ee = []
##ee.append(e(b"\x00"*15 + b"\x00").hex())
##ee.append(e(b"\x00"*15 + b"\x01").hex())
#ee.append(e(b"\x00"*15 + b"\x02").hex())
#ee.append(e(b"\x00"*15 + b"\x04").hex())

##ee.append(e(b"\x00"*7 + b"\x00" + b"\x00"*8).hex())
##ee.append(e(b"\x00"*7 + b"\x01" + b"\x00"*8).hex())
#ee.append(e(b"\x00"*15 + b"\x02").hex())
#ee.append(e(b"\x00"*15 + b"\x04").hex())
##pp.pprint(ee)











##diffs = {}
##printables = string.printable.strip(string.whitespace)
##for x1 in printables:
##    for x2 in printables:
##        for x3 in printables:
##            i1 = ord(x1)
##            i2 = ord(x2)
##            i3 = ord(x3)
##
##            prefix = b"crypto{" + i1.to_bytes(1, "little") + i2.to_bytes(1, "little") + i3.to_bytes(1, "little")
##            #print(prefix)
##            p = prefix + b"\x00" * (16 - len(prefix))
##
##            ee = e(p).hex()
##            diffs[prefix] = hex_diff(flag, ee)
##                
##
##temp = max(diffs.values())
##res = [key for key in diffs if diffs[key] == temp]
# printing result 
##print("Keys with maximum values are : " + str(res) + " val " + str(temp))


